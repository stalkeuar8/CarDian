...
from app.settings.database import celery_async_session_factory
...
# from app.schemas.verdicts_schemas import 
...
async def async_process_manual_lookup(lookup_id: int) -> None:
    ...
    car_info_to_predict = BasePredictor(**lookup_info.model_dump())
    
    predicted_price = await predict_service.predict(data_to_predict=car_info_to_predict)
