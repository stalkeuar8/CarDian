...
async def get_users_manual_lookup_by_id(lookup_id: int, session: AsyncSession = Depends(get_db), current_user: Users = Depends(get_current_user)) -> ManualLookupResponseSchema:
...
async def get_all_users_manual_lookups(current_user: Users = Depends(get_current_user), session: AsyncSession = Depends(get_db)) -> SequenceManualLookupResponseSchema:
